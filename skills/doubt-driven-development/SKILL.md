...
- **If you find yourself applying this skill from inside a subagent context** (where your current tool cannot spawn a nested fresh-context reviewer): the preferred path is to surface to the user that doubt-driven cannot run nested and let the main session handle it. As a last resort only, a degraded self-questioning fallback exists — rewrite ARTIFACT + CONTRACT as a fresh self-prompt with a hard mental separator from your prior reasoning, and walk Steps 1–5. This is **not fresh-context review** (you carry your own context with you), so flag the result as degraded and prefer escalation whenever the user is reachable.
...
When your target tool supports isolated persona runs, the role-based reviewers in `agents/` are usable here — see `agents/` for the roster and per-domain match.
